...
import { loadColumns } from './configureEstadisticasCols';
...
const TableSampleEstadisticas = ({
  ...
  const {
    ...
    loading,
    count,
    notify: estadisticasNotify,
    refetch,
  } = useAppSelector((state) => state.estadisticas);
  ...
  useEffect(() => {
    if (estadisticasNotify.showNotification) {
      notify(
        estadisticasNotify.typeNotification,
        estadisticasNotify.textNotification,
      );
    }
  }, [estadisticasNotify.showNotification]);
...
export default TableSampleEstadisticas;
