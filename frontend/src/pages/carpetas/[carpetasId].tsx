...
const EditCarpetas = () => {
  ...
  const { carpetasId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: carpetasId }));
  }, [carpetasId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: carpetasId, data }));
...
EditCarpetas.getLayout = function getLayout(page: ReactElement) {
  return <LayoutAuthenticated>{page}</LayoutAuthenticated>;
};

export default EditCarpetas;
