...
const EditTipos_doc = () => {
  ...
  const { tipos_docId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: tipos_docId }));
  }, [tipos_docId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: tipos_docId, data }));
...
EditTipos_doc.getLayout = function getLayout(page: ReactElement) {
  return <LayoutAuthenticated>{page}</LayoutAuthenticated>;
};

export default EditTipos_doc;
