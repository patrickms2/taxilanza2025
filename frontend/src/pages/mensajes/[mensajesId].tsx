...
const EditMensajes = () => {
  ...
  const { mensajesId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: mensajesId }));
  }, [mensajesId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: mensajesId, data }));
...
EditMensajes.getLayout = function getLayout(page: ReactElement) {
  return <LayoutAuthenticated>{page}</LayoutAuthenticated>;
};

export default EditMensajes;
