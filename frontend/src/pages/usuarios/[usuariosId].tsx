...
const EditUsuarios = () => {
  ...
  const { usuariosId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: usuariosId }));
  }, [usuariosId]);
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: usuariosId, data }));
...
EditUsuarios.getLayout = function getLayout(page: ReactElement) {
